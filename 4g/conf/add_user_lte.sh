...
# Add new user
if [[ -f ${DDBB_FILE} ]]; then
  last_user_name_number=$(grep -E -o "ue[1-9]{1,2}" ./${DDBB_FILE} | uniq | sort | tail -n 1 | awk -F "ue" '{print $2}')
  (( last_user_name_number += 1 ))
  insert_db="ue${last_user_name_number},mil,__IMSI__,__KI__,opc,__OPC__,8000,00000000230e,7,dynamic"
  # Check if string __NEW_USER__ exists, if not add it
  if [[ $(grep -c "# __NEW_USER__" ./${DDBB_FILE}) -eq 0 ]]; then
    echo "# __NEW_USER__" >> ./${DDBB_FILE}
  fi
  sed -i "s/# __NEW_USER__/${insert_db}\n# __NEW_USER__/g" ./${DDBB_FILE}
else
  insert_db="ue2,mil,__IMSI__,__KI__,opc,__OPC__,8000,00000000230e,7,dynamic"
